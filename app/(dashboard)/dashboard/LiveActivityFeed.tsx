...
export default function LiveActivityFeed({ activity, statusConfig }: Props) {
  const [feedFilter, setFeedFilter] = useState<'all' | 'messages' | 'payments' | 'automations'>('all');
  ...
  const filteredActivity = activity.filter((a) => {
    ...
    if (feedFilter === 'automations') return a.text.toLowerCase().includes('automation');
    return true;
  });
    ...
    <div className="dash-feed-card">
      <div className="dash-feed-header">
        ...
        <div className="dash-feed-filters">
          {(['all', 'messages', 'payments', 'automations'] as const).map((f) => (
            <button
              key={f}
